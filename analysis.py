...
with tabs[3]:
    ...
    # Clean + dedupe symbols before reindex (prevents "duplicate labels" crash)
    def _clean_sym(x):
        if pd.isna(x):
            return None
        return str(x).strip().upper()

    snap["Symbol"] = snap["Symbol"].map(_clean_sym)
    snap = snap.dropna(subset=["Symbol"])
    snap = snap.drop_duplicates(subset=["Symbol"], keep="first")

    # Ensure valid_syms is clean & unique
    valid_syms = [ _clean_sym(x) for x in valid_syms if pd.notna(x) ]
    valid_syms = pd.Index(valid_syms).unique().tolist()
